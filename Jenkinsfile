            ...
            steps{
               script{ 
                 withSonarQubeEnv(credentialsId: 'sonar-token') {
                     sh 'chmod +x mvnw'
                     sh './mvnw sonar:sonar -Dsonar.host.url=http://34.133.186.104:9000 -Dsonar.login=f5f186e6b3c2cad9a4535bc5ea2798d8e21bd51d'
                 }
        ...
        steps{
            script{
                   ...
                   withCredentials([kubeconfigFile(credentialsId: 'kube-gke', variable: 'KUBECONFIG')]) {
                    dir('Kubernetes/') {
                         sh 'helm upgrade --install --set image.tag="${VERSION}" devops-app devops-helm/ '
